...
export class TotalPostsSplitByWeekNumber extends ToString implements GroupByObj {
    categorical: string;
    numerical: number;
...
export class TotalPostsSplitByWeekNumberImpl extends Statistics {
    group(): Array<GroupByObj> {
        const totalPostsByWeek = this.posts.reduce( (acc: Record<string, number>, post: Post) => {
