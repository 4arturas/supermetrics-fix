...
export class AverageNumberOfPostsPerUserPerMonthImpl extends Statistics {
  group(): Array<GroupByObj> {
    const postsPerUser = this.posts.reduce(
      (acc: Record<string, Array<string>>, post: Post) => {
    ...
    const postsPerUserPerMonth = Object.keys(postsPerUser).map((from_id) => {
      return {
        from_id: from_id,
        postsPerMonth: postsPerUser[from_id].reduce(
          (acc: Record<string, number>, created_time: string) => {
    ...
    return postsPerUserPerMonth.map((e) => {
      return new AverageNumberOfPostsPerUserPerMonth(
        e.from_id,
        avg(
          Object.keys(e.postsPerMonth).map((month) => {
