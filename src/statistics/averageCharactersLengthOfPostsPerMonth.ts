...
export class AverageCharLengthOfPostsPerMonthImpl extends Statistics {
  group(): Array<GroupByObj> {
    const totalCharactersLengthPerMonth = this.posts.reduce(
      (acc: Record<string, Array<number>>, post: Post) => {
    ...
    return Object.keys(totalCharactersLengthPerMonth).map((month) => {
      const averageCharacterLength =
        totalCharactersLengthPerMonth[month].length === 0
          ? 0
          : totalCharactersLengthPerMonth[month].reduce(
              (previous: number, current: number) => previous + current,
              0
            ) / totalCharactersLengthPerMonth[month].length;
      return new AverageCharLengthOfPostsPerMonth(
        month,
